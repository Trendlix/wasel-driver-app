...
class InboxServiceLocator implements ServiceLocator {
  ...
  Future<void> initServiceLocator() async {
    ...
    // register mark all inboxes usecase
    if (!sl.isRegistered<MarkAllInboxesUsecase>()) {
      sl.registerLazySingleton<MarkAllInboxesUsecase>(
        () => MarkAllInboxesUsecase(sl()),
      );
    }

    if (!sl.isRegistered<MarkInboxAsReadUsecase>()) {
      sl.registerLazySingleton<MarkInboxAsReadUsecase>(
        () => MarkInboxAsReadUsecase(sl()),
      );
    }
    ...
    if (!sl.isRegistered<InboxCubit>()) {
      sl.registerFactory<InboxCubit>(
        () => InboxCubit(sl(), sl(), sl(), sl(), sl(), sl(), sl(), sl()),
      );
    }
  }
}
